  ...
  return (
    <main className="grain min-h-screen">
      <div className="mx-auto max-w-3xl px-6 py-16 md:px-10">
        ...
        <h1 className="mt-8 font-[family-name:var(--font-display)] text-5xl font-bold tracking-tight text-[var(--ink)]">
          Documentación
        </h1>
        ...
        <div className="mt-12 space-y-8">
          {docs.map((doc) => (
            <a
              key={doc.title}
              href={doc.href}
              className="block border-t border-[var(--line)] pt-6 transition hover:text-[var(--brass-deep)]"
              target="_blank"
              rel="noreferrer"
            >
              ...
              <p className="mt-2 text-[var(--ink-soft)]">{doc.body}</p>
            </a>
          ))}
        </div>
      </div>
