...
- Lint: `npm run lint`. Typecheck: `npm run typecheck`. Tests: `npm test`. Build: `npm run build` (runs `compile:contracts` then `next build`). Env presence audit (no secrets): `npm run check:env`.
